            ...
            <div class="row ">
                <div class="col-md-9 comp-grid " >
                    ...
                    <div  class="card-1 border rounded page-content" >
                        <!--[form-start]-->
                        <form id="ledgers-addcustomer-form" role="form" novalidate enctype="multipart/form-data" class="form page-form form-vertical needs-validation" action="{{ route('ledgers.addcustomer_store') }}" method="post">
                            @csrf
                            <div>
                                <div class="row">
                                    <div class="form-group col-sm-6">
                                        ...
                                        <div id="ctrl-sub_account_group_id-holder" class=" "> 
                                            <select required=""  id="ctrl-sub_account_group_id" data-field="sub_account_group_id" name="sub_account_group_id"  placeholder="{{ __('selectSundryDebtor') }}"    class="form-select" >
                                            <option value="">{{ __('selectSundryDebtor') }}</option>
                                            <?php 
                                                $options = $comp_model->ledgers_sub_account_group_id_option_list() ?? [];
                                    ...
                                    <div class="form-group col-sm-6">
                                        <label class="control-label" for="ledger_name">{{ __('customersName') }} <span class="text-danger">*</span></label>
                                        <div id="ctrl-ledger_name-holder" class=" "> 
                                            <input id="ctrl-ledger_name" data-field="ledger_name"  value="<?php echo get_value('ledger_name') ?>" type="text" placeholder="{{ __('enterCustomersName') }}"  required="" name="ledger_name"  class="form-control " />
                                        </div>
                                    </div>
                                    <div class="form-group col-sm-6">
                                        <label class="control-label" for="marketer_id">{{ __('marketerName') }} <span class="text-danger">*</span></label>
                                        <div id="ctrl-marketer_id-holder" class=" "> 
                                            <select required=""  id="ctrl-marketer_id" data-field="marketer_id" name="marketer_id"  placeholder="{{ __('selectMarketerName') }}"    class="form-select" >
                                            <option value="">{{ __('selectMarketerName') }}</option>
                                            <?php 
                                                $options = $comp_model->ledgers_marketer_id_option_list() ?? [];
                                                foreach($options as $option){
                                                $value = $option->value;
                                                $label = $option->label ?? $value;
                                ...
                                <input id="ctrl-code" data-field="code"  value="<?php echo get_value('code', "2011") ?>" type="hidden" placeholder="{{ __('enterCode') }}"  required="" name="code"  class="form-control " />
                            </div>
