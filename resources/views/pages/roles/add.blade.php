...
<?php
    $pageTitle = __('addNewRoles'); //set dynamic page title
?>
@extends($layout)
            ...
            <div class="row ">
                <div class="col-md-9 comp-grid " >
                    ...
                    <div  class="card-1 border rounded page-content" >
                        <!--[form-start]-->
                        <form id="roles-add-form" role="form" novalidate enctype="multipart/form-data" class="form page-form form-vertical needs-validation" action="{{ route('roles.store') }}" method="post">
                            @csrf
                            <div>
                            ...
                            <div class="bg-light p-2 subform">
                                <h4 class="record-title">Add New Permissions</h4>
                                <hr />
                                @csrf
                                <div>
                                    ...
                                    <td>
                                        ...
                                        <select required=""  id="ctrl-permission-row<?php echo $row; ?>" data-field="permission" name="permissions[<?php echo $row ?>][permission]"  placeholder="{{ __('selectAValue') }}"    class="selectize" >
                                ...
                                <input id="ctrl-company_id-row<?php echo $row; ?>" data-field="company_id"  value="<?php echo get_value('company_id', auth()->user()->company_id) ?>" type="hidden" placeholder="{{ __('enterCompanyId') }}" list="company_id_list"  required="" name="permissions[<?php echo $row ?>][company_id]"  class="form-control " />
                                <datalist id="company_id_list">
                                <?php 
                                    $options = $comp_model->company_id_option_list() ?? [];
                                    foreach($options as $option){
