...
use App\Http\Requests\Transaction_Ledgersadd4debitRequest;
...
class Transaction_LedgersController extends Controller
{
     ...
     */
	function add4contra(){
		return view("pages.transaction_ledgers.add4contra");
	}
     ...
     */
	function add4debit(){
		return view("pages.transaction_ledgers.add4debit");
	}
     ...
     * @return \Illuminate\Http\Response
     */
	function add4debit_store(Transaction_Ledgersadd4debitRequest $request){
		$postdata = $request->input("row");
		$modeldata = array_values($postdata);
		Transaction_Ledgers::insert($modeldata);
		return $this->redirect("transaction_ledgers", __('recordAddedSuccessfully'));
